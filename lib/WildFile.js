...
var WildFile = (function(){
	...
	class Filelist {
		...
		static option_keys = ['onadd','onupload','onprogress','oncomplete','onlistfinish'];
		...
		upload(url){
			...
			const onlistfinish = this.get_option('onlistfinish');
			if(onlistfinish){
				Promise.allSettled(complete_promises).then(results=>{
					...
					onlistfinish(this, successful_files, results.length);
				})
			}
		}
	}
